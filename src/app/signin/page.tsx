...
//     email,
//     password,
//     callbackUrl: "/admin", // Always send here
//   });
...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // 1️⃣ Pre-check role from backend
      const res = await fetch(
        "https://a2sv-application-platform-backend-team10.onrender.com/auth/token/check-role",
        {
          method: "POST",
      ...
      let userRole = roleData?.data?.role || "user";

      // 2️⃣ Sign in and send role in callbackUrl
      await signIn("credentials", {
        redirect: true,
        email,
        password,
        callbackUrl: `/?role=${userRole}`, // 👈 pass role here
      });
    } catch (err) {
      console.error(err);
