...
const EditUserPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    fullName: "",
    email: "",
    role: "applicant", // Default to a valid role
  });
  ...
  }, [id, sessionStatus, fetchUser, router]);
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/admin/users/${id}/`, {
        ...
        body: JSON.stringify({
          ...
          email: formData.email,
          role: formData.role,
          // Do not send password unless you intend to change it
        }),
      });
      ...
      if (!response.ok || !result.success) {
        ...
        const errorMessage =
          result.message || result.detail?.[0]?.msg || "Failed to update user";
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <main className="px-4 sm:px-8 md:px-12 lg:px-20 py-8">
        <h1 className="font-bold text-3xl">Edit User</h1>
        <p className="pb-5 text-gray-500">Update user information and role.</p>
